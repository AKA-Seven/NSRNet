...
import pdb
from models.operations import Operations as Operations, OPS
...
class hswish(nn.Module):  #做Hard Swish Function，它是谷歌开源的MobileNetV3中提出的激活函数
    def forward(self, x):
...
#         super(TaskAdaptor, self).__init__()
#         filter_num = in_c #256
#         self.conv1 = nn.Conv2d(in_c, 32, kernel_size=3, stride=2, padding=1, bias=False)
#         self.bn1 = nn.BatchNorm2d(32)
#         self.hs1 = hswish()

#         self.bneck = nn.Sequential(
#             Block(3, 32, 64, 32, hswish(), None, 1),
#             Block(3, 32, 128, 64, hswish(), None, 1),
#             Block(3, 64, 256, 128, hswish(), None, 1),
#             Block(3, 128, 512, filter_num, hswish(), SeModule(filter_num), 1),
#             Block(3, filter_num, 512, filter_num, hswish(), SeModule(filter_num), 1),
#         )
...
#         out = F.adaptive_avg_pool2d(out, (2, 2)) #b,256,2,2
#         out = out.view(out.size(0), -1) #b,256*4
...
class SRM(nn.Module):
    ...
    def forward(self,inp):
        ...
        task_logit = self.task_adaptor(inp_enc) #[3,3] [b,num_task] 需要return计算损失函数
